 ...
 const validateForm = (): boolean => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (imageFile) {
        // Send as multipart/form-data so the image travels as a proper file upload
        const fd = new FormData();
        fd.append('image', imageFile);
        // Append all other fields as a JSON blob so the backend can parse them
        fd.append('data', JSON.stringify(recipePayload));
        // Also spread flat fields for backends that read individual form fields
        Object.entries(recipePayload).forEach(([key, val]) => {
