          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {sortedRecipes.map((recipe, index) => (
                <RecipeSummaryCard key={`${recipe.recipe_id}-${index}`} recipe={recipe as any} />
              ))}
            </div>
          )}
        </div>
...
export default function FollowedChefsPage() {
  return (
    <Suspense fallback={<div className="p-12 text-center text-gray-500">Loading followed chefs...</div>}>
      <FollowedChefsContent />
    </Suspense>
  );
}
